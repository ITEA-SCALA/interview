...
Design patterns (шаблоны или дизайн патерны проектирования):
---
1. Структурные
2. Поведенческие
3. Порождающие
...
Порождающие:
---
1. Builder — разделяет создания объекта от его представления (позволяет пошагово создавать сложные объекты)
2. Прототип — клонирует объекты (новые объекты создаются по образу и подобию уже существующего)

3. Singleton — гарантирует единственный экземпляр класса в системе

4. Фабричный метод — создаёт объект через отдельный фабричный-метод (делегируем процесс создания объектов)
5. Абстрактная фабрика — создаёт объект через фабричную-сущность для создания группы-семейств взаимосвязанных объектов
