...
Так вот, чтобы не тратить время на создание пула-потоков - и существуют **wait/notify-блокировки** для управления этими потоками.
...
1. Если *внутри одного сервиса выполняется много под-задач*:
   - в этом случае нужно применять внутренние потоки (порождать программный пул-потоков), чтобы комбинировать под-задачи внутри сервиса (с помощью `Thread`,`ExecutorService`,`ForkJoinPool`)
     ```text
     А еще wait/notify-блокировки применяются, чтобы синхронизировать эти внутренние потоки для доступа к конкурентным данным, которые помечены оператором synchronized.
     Тоесть, когда внутри сервиса разбить одну-сложную задачу и выполнить ее частями параллельно в разных внутренних потоках, а потом эти результаты собрать.
     ```
2. Но если *сам сервис является конкурентным объектом* данных:
   - в этом случае нужно применять synchronized, для управлять внешними потоками (`http-request`) 
     ```text
     Внешними (http-request) потоками приложение не может управлять, потому-что их порождает сам клиент.
     Тоесть, здесь не получиться собрать результаты, потому-что потоки являются внешними.
...
Например:
```text
В главном потоке нужно создать (общий) 'Look-объект', через который потом можно будет управлять побочными потоками, с помощью -блокировок-wait/notify.
...
Внутри ExecutorService-а или CompletableFuture или concurrent-collections уже реализованы wait/notify-блокировки для управления потоками.
```
