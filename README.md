...
- Необязательно возвращает разные адреса для разных ключей
...
Каждый из бакетов представляет из себя коллекцию
...
1. StringBuilder — методы StringBuilder не синхронизированные и потоко-не-безопасные, поэтому операции с ними работают быстрее (поскольку не обременен синронизацией).
                   днопоточном окужении мы должны использовать StringBuilder
2. StringBuffer — методы StringBuffer синхронизированные и потоко-безопасные, поэтому операции с ними работают медленно.
